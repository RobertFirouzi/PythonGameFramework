...
class InputHandler:
    def __init__(self, game = None, player = None, buttonMap = None, inputType = PRAM.INPTYPE_OBSERVER):
        ...
        self.keydownEvents = list()
        self.keysPressed = None

    def appendKeydownEvent(self, event):
        self.keydownEvents.append(event)

    def setKeysPressed(self, keysPressed):
        self.keysPressed = keysPressed

    def handleInputs(self):
        while len(self.keydownEvents) > 0:
            event = self.keydownEvents.pop()
        ...
        if self.keysPressed[self.buttonMap.up]:
            self.inputUpBehavior()
        if self.keysPressed[self.buttonMap.down]:
            self.inputDownBehavior()
        if self.keysPressed[self.buttonMap.left]:
            self.inputLeftBehavior()
        if self.keysPressed[self.buttonMap.right]:
